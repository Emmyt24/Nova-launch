...
describe('Contract-Backend Consistency Tests', () => {
  describe('TokenInfo Field Consistency', () => {
    ...
    it('should have matching field types', () => {
      // String fields
      const stringFields = ['address', 'creator', 'name', 'symbol'];
      stringFields.forEach(field => {
        expect(typeof ({} as ContractTokenInfo)[field as keyof ContractTokenInfo]).toBe('string');
        expect(typeof ({} as BackendToken)[field as keyof BackendToken]).toBe('string');
      });

      // Number fields
      expect(typeof ({} as ContractTokenInfo).decimals).toBe('number');
      expect(typeof ({} as BackendToken).decimals).toBe('number');

      // BigInt fields
      expect(typeof ({} as ContractTokenInfo).total_supply).toBe('bigint');
      expect(typeof ({} as BackendToken).totalSupply).toBe('bigint');

      // Nullable string fields
      const nullableFields: Array<keyof ContractTokenInfo> = ['metadata_uri'];
      nullableFields.forEach(field => {
        const value = ({} as ContractTokenInfo)[field];
  ...
  describe('API Endpoint Consistency', () => {
    ...
    it('should have consistent request/response formats', () => {
      ...
      // Verify request/response types are defined
      expect(typeof ({} as CreateTokenRequest).creator).toBe('string');
      expect(typeof ({} as TokenResponse).address).toBe('string');
      expect(typeof ({} as BurnRequest).amount).toBe('bigint');
    });
  });
  ...
  describe('Event Structure Consistency', () => {
    it('should have matching event structures', () => {
      ...
      // Verify event structures are defined
      expect(typeof ({} as TokenCreatedEvent).token_address).toBe('string');
      expect(typeof ({} as BurnEvent).amount).toBe('bigint');
      expect(typeof ({} as AdminTransferEvent).new_admin).toBe('string');
      expect(typeof ({} as PauseEvent).paused).toBe('boolean');
      expect(typeof ({} as FeesUpdatedEvent).base_fee).toBe('bigint');
    });
  });
  ...
  describe('Address Format Consistency', () => {
    it('should validate Stellar address format', () => {
      ...
      expect(validateStellarAddress('GABC123DEFGHIJKLMNOPQRSTUVWXYZ234567890ABCDEFGHIJKLMNO')).toBe(true);
